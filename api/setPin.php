 ...
 function setPin(){
    ...
    if($customerId){
      ...
      $fields = file_get_contents('php://input'); 
       ...
       $sql = "select * from users where token = '".$token."' and status='1' "; 
          ...
          $sqlUpdate = "UPDATE users SET 
           ...
           where token = '".$token."'";
            if ($conn->query($sqlUpdate) === TRUE) {
                 $message = ["error_code"=>1,"message"=>'Pin set successfully','data'=>[]];
                  return json_encode($message); 
            }else{
                
                $message = ["error_code"=>0,"message"=>'Pin could not be updated','data'=>[]];
                  return json_encode($message);
